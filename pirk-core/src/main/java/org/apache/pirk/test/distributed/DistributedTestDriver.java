...
public class DistributedTestDriver
{
  ...
  public static void test(FileSystem fs, DistributedTestCLI cli, List<JSONObject> pirDataElements) throws Exception
  {
    ...
    List<String> platforms = ResponderService.getInstance().getPlatforms().stream().filter((ResponderPlugin plugin) -> plugin.hasDistributedTest())
        .map((ResponderPlugin plugin) -> plugin.getPlatformName()).collect(Collectors.toList());

    // MapReduce JSON input
    if (cli.run("1:J") && platforms.contains(MAPREDUCE))
    {
      DistTestSuite.testJSONInput(MAPREDUCE, fs, pirDataElements);
    }

    // Spark with JSON input
    if (cli.run("1:JS") && platforms.contains(SPARK))
    {
      DistTestSuite.testJSONInput(SPARK, fs, pirDataElements);
    }

    // Spark Streaming
    if (cli.run("1:SS") && platforms.contains(SPARKSTREAMING))
    {
      DistTestSuite.testJSONInput(SPARKSTREAMING, fs, pirDataElements);
    }

    if (cli.run("1:JSS") && platforms.contains(SPARKSTREAMING))
    {
      DistTestSuite.testJSONInput(SPARKSTREAMING, fs, pirDataElements);
    }
    ...
    if (cli.run("1:E") || cli.run("1:ES") || cli.run("1:ESS"))
    {
      ...
      if (cli.run("1:E"))
      {
        DistTestSuite.testESInput(MAPREDUCE, fs, pirDataElements);
      }
      if (cli.run("1:ES"))
      {
        DistTestSuite.testESInput(SPARK, fs, pirDataElements);
      }
      if (cli.run("1:ESS"))
      {
        DistTestSuite.testESInput(SPARKSTREAMING, fs, pirDataElements);
      }
    }
  }
