...
import org.apache.pirk.responder.wideskies.ResponderService;
...
public class BaseTests
{
  ...
  {
    testDNSHostnameQuery(dataElements, null, STANDALONE, false, numThreads, testFalsePositive);
  }

  public static void testDNSHostnameQuery(List<JSONObject> dataElements, FileSystem fs, String platformName, boolean isDistributed, int numThreads)
      throws Exception
  {
    testDNSHostnameQuery(dataElements, fs, platformName, isDistributed, numThreads, false);
  }
      ...
      boolean testFalsePositive) throws Exception
  {
    ...
    if (isDistributed)
    {
      results = performQuery(Inputs.DNS_HOSTNAME_QUERY, selectorsDomain, fs, platformName, numThreads);
    }
    else
    {
  ...
  private static ArrayList<String> constructBasicArgs()
  {
    ...
    args.add("-" + ResponderProps.OUTPUTFILE + "=" + SystemConfiguration.getProperty(DistributedTestDriver.OUTPUT_DIRECTORY_PROPERTY));
  ...
  public static List<QueryResponseJSON> performQuery(String queryType, ArrayList<String> selectors, FileSystem fs, String platformName, int numThreads)
      throws Exception
  {
    ...
    if (responder == null)
    {
      throw new PIRException("No such platform plugin found: " + platformName);
    }
    else if (!responder.hasDistributedTest())
    {
      throw new PIRException("No such distributed test for platform: " + platformName);
    }
    ...
    List results = responder.runDistributedTest(args, fs, querier, numThreads);
    return results;
  }
  ...
  public static void testDNSIPQuery(ArrayList<JSONObject> dataElements, int numThreads) throws Exception
  {
    ...
    testDNSIPQuery(dataElements, null, "standalone", false, numThreads);
  ...
  public static void testDNSIPQuery(List<JSONObject> dataElements, FileSystem fs, String platformName, boolean isDistributed, int numThreads) throws Exception
  {
    ...
    if (isDistributed)
    {
      results = performQuery(Inputs.DNS_IP_QUERY, selectorsIP, fs, platformName, numThreads);
  ...
  {
    testDNSNXDOMAINQuery(dataElements, null, STANDALONE, false, numThreads);
  }

  // A query that returned an nxdomain response was made for the watched hostname; watched value type: hostname (String)
  public static void testDNSNXDOMAINQuery(List<JSONObject> dataElements, FileSystem fs, String platformName, boolean isDistributed, int numThreads)
      throws Exception
  {
    ...
    if (isDistributed)
    {
      results = performQuery(Inputs.DNS_NXDOMAIN_QUERY, selectorsDomain, fs, platformName, numThreads);
    }
    else
    {
  ...
  {
    testSRCIPQuery(dataElements, null, STANDALONE, false, numThreads);
  }

  // Query for responses from watched srcIPs
  public static void testSRCIPQuery(List<JSONObject> dataElements, FileSystem fs, String platformName, boolean isDistributed, int numThreads) throws Exception
  {
    ...
    if (isDistributed)
    {
      results = performQuery(Inputs.DNS_SRCIP_QUERY, selectorsIP, fs, platformName, numThreads);
  ...
  // Query for responses from watched srcIPs
  public static void testSRCIPQueryNoFilter(List<JSONObject> dataElements, FileSystem fs, String platformName, boolean isDistributed, int numThreads)
      throws Exception
  {
    ...
    if (isDistributed)
    {
      results = performQuery(Inputs.DNS_SRCIP_QUERY_NO_FILTER, selectorsIP, fs, platformName, numThreads);
    }
    else
    {
